...
function setupSocketHandlers(io) {
  io.on('connection', socket => {
    ...
    socket.on('sync-check', ({ roomId, currentTime, isPlaying, timestamp }) => {
      try {
        const room = rooms.get(roomId);
        const user = users.get(socket.id);
        ...
        // Broadcast sync update to all other users in the room
    ...
    socket.on('typing-start', ({ roomId }) => {
      try {
        ...
        console.log(`${user.name} started typing in room ${roomId}`);
      } catch (error) {
    ...
    socket.on('typing-stop', ({ roomId }) => {
      try {
        ...
        console.log(`${user.name} stopped typing in room ${roomId}`);
      } catch (error) {
